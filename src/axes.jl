...
abstract type DIRECTIONS end
abstract type XYZ<:DIRECTIONS end
abstract type XY<:DIRECTIONS end
...
struct AXES{A<:Axis,B<:Axis,C<:Axis}
    ax1::A
    ax2::B
    ax3::C
end
...
Base.size(axes::AX, location) where {AX<:AXES}= Tuple(size(d,l) for (d,l) in zip([axes.ax1,axes.ax2,axes.ax3],location))
...
(::Type{CENTERS})(axes::AX) where {AX<:AXES} = Iterators.product(CENTERS(axes.ax1),CENTERS(axes.ax2),CENTERS(axes.ax3))

CCC = (CENTERS, CENTERS, CENTERS)
